...
export default function LeaderboardPage() {
  ...
  useEffect(() => {
    ...
    const loadContests = async () => {
      ...
      const result = await getLeaderboardContestsAction("upcoming");
      if (!mounted) return;
    ...
    <div className={`flex min-h-screen font-['Poppins'] ${isDark ? "bg-slate-950 text-slate-100" : "bg-gray-50"}`}>
      ...
      >
        ...
        >
          <div>
            ...
            <p className={`mt-1 text-sm ${isDark ? "text-slate-300" : "text-gray-600"}`}>
              Track rankings for upcoming contests.
            </p>
          </div>
          <ThemeToggle />
        </header>
        ...
        <div className="w-full space-y-6 p-8">
          ...
          <section
            className={`rounded-2xl border p-4 ${isDark ? "border-slate-700 bg-slate-900/95" : "border-gray-200 bg-white"}`}
          >
            <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
              <span
                className={`inline-flex w-fit rounded-xl border px-3 py-2 text-xs font-semibold uppercase tracking-wide ${
                  isDark ? "border-slate-600 bg-slate-800 text-slate-300" : "border-gray-300 bg-gray-50 text-gray-600"
                }`}
              >
                Completed Contests
              </span>
              ...
              <div className="flex items-center gap-3">
                ...
                <select
                  ...
                  className={`rounded-lg border px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-red-200 ${
                    isDark ? "border-slate-600 bg-slate-800 text-slate-200" : "border-gray-300 bg-white text-gray-700"
                  }`}
                >
          ...
          {selectedContest ? (
            <div className="grid grid-cols-1 gap-6 xl:grid-cols-3">
              ...
              >
                ...
                <div className={`mb-2 flex items-center gap-2 ${isDark ? "text-slate-100" : "text-gray-900"}`}>
                  <Trophy className="h-5 w-5 text-orange-600" />
                  <h3 className="text-xl font-bold">{selectedContest.matchLabel}</h3>
                </div>
                <p className={`text-sm ${isDark ? "text-slate-300" : "text-gray-700"}`}>
                  {new Date(selectedContest.startsAt).toLocaleString("en-US")}
                </p>
                ...
                <div className="mt-6 grid grid-cols-1 gap-4 md:grid-cols-3">
                  {screenLoading || loading ? (
                    ...
                    leaders.slice(0, 3).map((leader) => <PodiumCard key={leader.userId} leader={leader} />)
                  )}
                </div>
              </section>
              ...
              >
                ...
                <p className={`mt-1 text-sm ${isDark ? "text-slate-300" : "text-gray-600"}`}>
                  For {selectedContest.matchLabel}
                </p>

                {screenLoading || loading ? (
                  <div
                    className={`mt-5 h-36 animate-pulse rounded-2xl border ${
                  ...
                  >
                    ...
                    <p className={`mt-1 text-4xl font-black ${isDark ? "text-slate-100" : "text-gray-900"}`}>#{myEntry.rank}</p>
                    <p className={`mt-2 text-sm ${isDark ? "text-slate-300" : "text-gray-600"}`}>
                      {myEntry.pts} pts · {myEntry.winRate}% win rate
                    </p>
                ...
                <div className="overflow-x-auto">
                  <table className="min-w-[760px] w-full">
                    <thead className={isDark ? "bg-slate-800/80" : "bg-gray-50"}>
                      <tr>
                        <th className={`px-6 py-3 text-left text-xs font-semibold uppercase tracking-wide ${isDark ? "text-slate-300" : "text-gray-600"}`}>Rank</th>
                        <th className={`px-6 py-3 text-left text-xs font-semibold uppercase tracking-wide ${isDark ? "text-slate-300" : "text-gray-600"}`}>Player</th>
                        <th className={`px-6 py-3 text-left text-xs font-semibold uppercase tracking-wide ${isDark ? "text-slate-300" : "text-gray-600"}`}>Teams</th>
                        <th className={`px-6 py-3 text-left text-xs font-semibold uppercase tracking-wide ${isDark ? "text-slate-300" : "text-gray-600"}`}>Points</th>
                        <th className={`px-6 py-3 text-left text-xs font-semibold uppercase tracking-wide ${isDark ? "text-slate-300" : "text-gray-600"}`}>Win Rate</th>
                        <th className={`px-6 py-3 text-left text-xs font-semibold uppercase tracking-wide ${isDark ? "text-slate-300" : "text-gray-600"}`}>Prize</th>
                      </tr>
                    </thead>
                    <tbody>
                      {screenLoading || loading ? (
                        ...
                        leaders.map((leader) => (
                          ...
                          >
                            <td className="px-6 py-4">{rankBadge(leader.rank, isDark)}</td>
                            ...
                            <td className="px-6 py-4 font-semibold text-red-500">{leader.winRate}%</td>
                            <td className="px-6 py-4 font-semibold text-emerald-600">{formatCredits(leader.prize)}</td>
                          </tr>
                        ))
                      )}
                    </tbody>
...
function StatCard({ label, value }: { label: string; value: string }) {
  const { isDark } = useThemeMode();
  return (
    <div className={`rounded-2xl border p-4 ${isDark ? "border-slate-600 bg-slate-900" : "border-gray-200 bg-white/80"}`}>
      <p className={`text-xs ${isDark ? "text-slate-400" : "text-gray-600"}`}>{label}</p>
      <p className={`mt-1 text-xl font-bold ${isDark ? "text-slate-100" : "text-gray-900"}`}>{value}</p>
    </div>
  );
}
...
function rankBadge(rank: number, isDark: boolean) {
  ...
  return <span className={`font-medium ${isDark ? "text-slate-300" : "text-gray-700"}`}>#{rank}</span>;
}
...
function PodiumCard({ leader }: { leader: LeaderItem }) {
  ...
  return (
    <div className={`rounded-2xl border p-4 ${tone}`}>
      ...
      <p className={`mt-1 text-lg font-bold ${isDark ? "text-slate-100" : "text-gray-900"}`}>{leader.name}</p>
      <p className={`mt-2 text-sm ${isDark ? "text-slate-300" : "text-gray-700"}`}>{leader.pts} points</p>
      <p className={`mt-1 text-sm font-semibold ${isDark ? "text-emerald-300" : "text-emerald-700"}`}>
        Prize: {formatCredits(leader.prize)}
      </p>
    </div>
  );
}
...
function PodiumSkeleton() {
  const { isDark } = useThemeMode();
  return (
    <div className={`h-28 animate-pulse rounded-2xl border ${isDark ? "border-slate-700 bg-slate-800" : "border-gray-200 bg-white/70"}`} />
  );
}
...
function LeaderboardRowSkeleton() {
  ...
  return (
    <tr className={`border-t ${isDark ? "border-slate-800" : "border-gray-100"}`}>
      <td className="px-6 py-4">
        <div className={`h-5 w-10 animate-pulse rounded ${isDark ? "bg-slate-700" : "bg-gray-200"}`} />
      </td>
      <td className="px-6 py-4">
        <div className={`h-6 w-40 animate-pulse rounded ${isDark ? "bg-slate-700" : "bg-gray-200"}`} />
      </td>
      <td className="px-6 py-4">
        <div className={`h-5 w-8 animate-pulse rounded ${isDark ? "bg-slate-700" : "bg-gray-200"}`} />
      </td>
      <td className="px-6 py-4">
        <div className={`h-5 w-14 animate-pulse rounded ${isDark ? "bg-slate-700" : "bg-gray-200"}`} />
      </td>
      <td className="px-6 py-4">
        <div className={`h-5 w-12 animate-pulse rounded ${isDark ? "bg-slate-700" : "bg-gray-200"}`} />
      </td>
      <td className="px-6 py-4">
        <div className={`h-5 w-16 animate-pulse rounded ${isDark ? "bg-slate-700" : "bg-gray-200"}`} />
      </td>
    </tr>
  );
}
