...
export function decodeSelectedIds(value: string | null): string[] {
  if (!value) {
    return [];
  }
...
export function encodeSelectedIds(ids: string[]): string {
  return ids.join(",");
}
...
export function isValidTeamComposition(
  selectedIds: string[],
  availablePlayers: Player[] = players
): boolean {
  ...
  const counts = getRoleCounts(selectedIds, availablePlayers);
...
export function mapBackendRoleToUiRole(role: string): Role {
